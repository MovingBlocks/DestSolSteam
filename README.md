## Destination Sol on Steam

This project provides the Steam front-end for [Destination Sol](https://github.com/MovingBlocks/DestinationSol) and is meant to be used from within that project, not independently. See its repo for basic setup instructions!

### Usage
- For local testing, you will need to create a `steam_appid.txt` file in the root project directory (see [here](https://partner.steamgames.com/doc/sdk/api#SteamAPI_Init) for more details).
...
#### Upload Process
1. Copy the build scripts from this repository to the steam-sdk directory at: steam-sdk/tools/ContentBuilder/scripts
2. Get the Steam build that you want to use. This is most likely the latest release build from Jenkins: https://jenkins.terasology.io/job/DestinationSol/job/engine/
